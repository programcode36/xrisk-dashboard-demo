...
extrae el bloque de PASIVO (+ Patrimonio + Contingentes) por empresa
bancaria: desde "OBLIGACIONES CON EL PÚBLICO" hasta "Otras Cuentas
Contingentes", inclusive. Es el mismo archivo/hoja que usa
descarga_sbs_activos.py, pero mas abajo.

Formato real del archivo (verificado contra B-2201-jl2026.XLS y
B-2201-ag2012.XLS):
- El bloque de Pasivo tiene su PROPIO encabezado de bancos (fila "Pasivo"
  + nombre de cada banco, y debajo la fila MN/ME/TOTAL), separado e
  independiente del encabezado del bloque de Activo. El ORDEN de los
  bancos puede ser distinto entre el bloque de Activo y el de Pasivo del
  mismo archivo (se vio en la practica), asi que no se puede reusar el
  encabezado de Activo: hay que ubicar este de nuevo, mas abajo en la hoja.
- Igual que en Activo, la columna con el nombre de cuenta no siempre es la
  A: puede estar corrida a la B u otra. Se detecta dinamicamente buscando
  el texto "OBLIGACIONES CON EL PUBLICO" en las columnas anteriores al
  primer bloque de banco.
- Entre "TOTAL ACTIVO" y el encabezado de Pasivo puede haber, segun el año,
  lineas intermedias (tipo de cambio, un mini-bloque "CONTINGENTES
  DEUDORAS", titulo repetido, etc.) que se ignoran: la busqueda del
  encabezado de Pasivo escanea hacia abajo hasta encontrar una fila con
  las etiquetas MN/ME/TOTO, sin asumir un offset fijo de filas.
- El bloque copiado (OBLIGACIONES CON EL PUBLICO ... Otras Cuentas
  Contingentes) de punta a punta incluye, ademas del Pasivo propiamente
  dicho, las filas de PATRIMONIO y de CONTINGENTES que estan impresas en
  el medio, tal como pidio el usuario.
- Igual que en el script de activos: NO se asume que la cantidad/orden de
  cuentas sea igual entre meses distintos. Cada mes aporta sus propias
  cuentas tal cual vienen en su archivo, y el consolidado final copia cada
...
Uso:
    python descarga_sbs_pasivos.py                       # descarga todo el rango 2002-01 a 2026-07
    python descarga_sbs_pasivos.py --start 2020-01 --end 2021-12
    python descarga_sbs_pasivos.py --inspect 2026-07      # solo descarga y vuelca el crudo de un mes
    python descarga_sbs_pasivos.py --local-file ruta.xls --fecha 2026-07  # parsea un archivo ya descargado, sin red
"""
...
# Palabras clave que delimitan el bloque a copiar (normalizadas: mayusculas,
# sin tildes). Inclusive en ambos extremos.
PALABRA_INICIO = "OBLIGACIONES CON EL PUBLICO"
PALABRA_FIN = "OTRAS CUENTAS CONTINGENTES"
...
def _leer_hoja_balance(path: Path) -> pd.DataFrame:
    ...
    xlsx real, xls binario clasico, o HTML disfrazado de .xls, ni como se
    llame la hoja ("1" en archivos recientes, "05-BG"/"05-BG (P)"/"BG" en
    antiguos)."""
...
def _buscar_texto_en_columna(df: pd.DataFrame, col: int, texto_normalizado: str, desde_fila: int = 0):
    """Devuelve el indice de la primera fila (desde `desde_fila`) donde la
    columna `col`, normalizada (sin tildes/mayusculas/strip), sea igual al
    texto buscado. None si no aparece."""
...
def _ubicar_bloque_pasivo(df: pd.DataFrame) -> tuple[int, int, int, int, int]:
    """
    Devuelve (fila_banco, fila_submoneda, fila_inicio, fila_fin, col_cuenta)
    para el bloque de Pasivo (que empieza en PALABRA_INICIO y termina en
    PALABRA_FIN), buscando su propio encabezado de bancos de forma
    independiente al de Activo (puede estar en otro orden de bancos, y a
    una distancia variable de filas desde donde termina el Activo).
    """
    col0 = df.iloc[:, 0].map(_sin_tildes)
    fila_total_activo = _buscar_texto_en_columna(df, 0, "TOTAL ACTIVO")
    desde = (fila_total_activo + 1) if fila_total_activo is not None else 0

    fila_submoneda = None
    for r in range(desde, len(df)):
        valores = df.iloc[r].astype(str).str.strip().str.upper()
        if (valores == "MN").sum() >= 2 and (valores == "ME").sum() >= 2:
            fila_submoneda = r
            break
    if fila_submoneda is None:
        raise ValueError("No se encontro la fila de encabezado MN/ME/TOTAL del bloque de Pasivo")
    ...
    if primer_col_banco is None:
        raise ValueError("No se encontro ninguna columna 'MN' en la fila de sub-encabezado de Pasivo")
    ...
    fila_fin = _buscar_texto_en_columna(df, col_cuenta, PALABRA_FIN, desde_fila=fila_inicio + 1)
    if fila_fin is None:
        raise ValueError(f"No se encontro '{PALABRA_FIN}' despues de la fila {fila_inicio}")

    return fila_banco, fila_submoneda, fila_inicio, fila_fin, col_cuenta


def extraer_pasivos(df: pd.DataFrame, fecha) -> pd.DataFrame:
    """
    Extrae, de la hoja de Balance General, el bloque desde "OBLIGACIONES
    CON EL PUBLICO" hasta "Otras Cuentas Contingentes" (inclusive) por
    empresa bancaria, en formato largo:
    fecha, banco, es_total, cuenta, moneda_nacional, moneda_extranjera, total
    """
    fila_banco, fila_submoneda, fila_inicio, fila_fin, col_cuenta = _ubicar_bloque_pasivo(df)
    ...
    # Mismo criterio que en activos: las filas separadoras se identifican
    # por el TEXTO de la columna de cuenta (blanco para todos los bancos
    # por igual), no por los valores numericos de cada banco (que pueden
    # venir en blanco/guion en una cuenta real sin ser separador). Y no se
    # compara via astype(str) == "nan" porque con pandas >= 2.x/3.x un NaN
    # convertido a texto ya no es comparable con la cadena "nan".
    ...
    if not bloques:
        raise ValueError("No se detecto ningun bloque de banco (MN/ME/TOTAL) en el bloque de Pasivo")
...
def a_formato_ancho(largo: pd.DataFrame) -> pd.DataFrame:
    ...
    contable. Se arma por POSICION (no por nombre de cuenta), porque
    algunos nombres se repiten dentro del mismo mes.
    """
...
def guardar_excel(df: pd.DataFrame, destino: Path) -> None:
    ...
    df.to_excel(destino, index=False, sheet_name="Pasivos")
...
def guardar_excel_por_bloques(piezas: list[pd.DataFrame], destino: Path) -> None:
    ...
    (sin alinear cuentas de distintos meses por nombre). Se escribe directo
    con openpyxl porque cada mes puede tener una cantidad de columnas
    distinta y porque hay nombres de cuenta repetidos dentro de un mismo
    mes (validos como texto en una fila, no como Index de columnas de
    pandas).
    """
...
def main():
    ...
    if args.local_file:
        ...
        pasivos = extraer_pasivos(df_crudo, ultimo_dia_mes(year, month))
        ancho = a_formato_ancho(pasivos)
        destino = salida / f"pasivos_{args.fecha}.xlsx"
    ...
    if args.inspect:
        ...
        volcado = salida / f"inspect_pasivos_{year}-{month:02d}.csv"
    ...
    destino = salida / "pasivos_bancos_2002_2026.xlsx"
